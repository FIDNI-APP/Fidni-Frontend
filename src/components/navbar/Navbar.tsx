    ...
    <div className="relative">
      {hasDropdown ? (
        ...
        >
          ...
          {(isActive || activeDropdown === dropdown) && (
            <div className={`absolute bottom-0 left-1/2 -translate-x-1/2 w-1/2 h-0.5 rounded-full transition-all duration-200 ${getActiveBarGradient()}`} />
          )}
        </button>
      ) : (
        <Link
    ...
    if (hasDropdown) {
      return (
        <div>
          ...
          {isExpanded && (
            <div className="ml-6 mt-2 space-y-1 border-l-2 pl-3 border-gray-300">
              {isLoading ? (
                <div className="py-3 px-4 flex items-center text-gray-600">
                  <div className="animate-spin rounded-full h-4 w-4 border-b-2 inline-block mr-2 border-gray-600"></div>
                  Chargement...
                </div>
              ) : (
                <>
                  <Link
                    to={`/${dropdown}`}
                    className="block w-full px-4 py-2.5 text-sm rounded-lg hover:bg-gray-100 transition-all duration-150 font-medium text-gray-900"
                  ...
                  {mobileSubItems.map((item) => (
                    <Link
                      ...
                      className="block w-full px-4 py-2.5 text-sm rounded-lg hover:bg-gray-100 transition-all duration-150 text-gray-600"
                      onClick={() => setMobileMenuOpen(false)}
                    >
                      {item.name}
                    </Link>
        ...
        <div className="flex items-center justify-between gap-8">
          ...
          <div className="hidden md:block w-full max-w-md mx-6">
            <SearchAutocomplete
              ...
              type={getContentType()}
              inputClassName="w-full px-4 py-2 pr-20 rounded-xl focus:outline-none transition-all duration-300 bg-gray-50 text-gray-900 border border-gray-200 focus:ring-2 focus:ring-purple-500 placeholder-gray-400"
            />
          </div>
        ...
        <div className="md:hidden transition-all duration-300 ease-in-out bg-white border-t border-gray-200">
          <div className="max-w-7xl mx-auto px-5 md:px-10 lg:px-16 py-6 space-y-6">
            ...
            <SearchAutocomplete
              ...
              inputClassName="w-full px-4 py-3 pr-20 rounded-xl focus:outline-none focus:ring-2 transition-all duration-300 bg-gray-50 text-gray-900 border border-gray-200 focus:ring-purple-500 placeholder-gray-400"
            />
              ...
              <div className="flex flex-col space-y-3">
                <div className="flex items-center space-x-3 px-4 py-3 rounded-xl bg-gray-100">
                  <img
                    ...
                    alt="Profile"
                    className="w-12 h-12 rounded-full border-2 shadow-lg border-gray-300"
                  />
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold text-base truncate text-gray-900">{user?.username}</div>
                    <div className="text-sm truncate text-gray-600">{user?.email}</div>
                  </div>
                </div>

                <Link to={`/profile/${user?.username}`} className="flex items-center w-full px-4 py-3 rounded-xl transition-all duration-200 font-medium text-gray-900 hover:bg-gray-100" onClick={() => setMobileMenuOpen(false)}>
                ...
                <Link to="/saved" className="flex items-center w-full px-4 py-3 rounded-xl transition-all duration-200 font-medium text-gray-900 hover:bg-gray-100" onClick={() => setMobileMenuOpen(false)}>
                  <div className="p-2 rounded-lg mr-3 bg-gray-200">
                ...
                <Link to="/revision-lists" className="flex items-center w-full px-4 py-3 rounded-xl transition-all duration-200 font-medium text-gray-900 hover:bg-gray-100" onClick={() => setMobileMenuOpen(false)}>
                  <div className="p-2 rounded-lg mr-3 bg-gray-200">
                ...
                <Link to="/settings" className="flex items-center w-full px-4 py-3 rounded-xl transition-all duration-200 font-medium text-gray-900 hover:bg-gray-100" onClick={() => setMobileMenuOpen(false)}>
                  <div className="p-2 rounded-lg mr-3 bg-gray-200">
                ...
                <button
                  ...
                  className="flex items-center w-full px-4 py-3 rounded-xl transition-all duration-200 font-medium text-red-600 hover:text-red-700 hover:bg-red-50"
                >
