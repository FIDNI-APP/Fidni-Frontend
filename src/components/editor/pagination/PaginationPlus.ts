...
import { updateCssVariables } from "./paginationUtils";
  ...
  const pageWidget = Decoration.widget(
    ...
    (view) => {
      ...
      const pageBreakDefinition = ({
        firstPage = false,
        pageIndex = 0,  // Add this parameter
      }: {
        firstPage: boolean;
        pageIndex?: number;  // Add this parameter
      }) => {
        const pageContainer = document.createElement("div");
        pageContainer.classList.add("rm-page-break");
        pageContainer.dataset.pageIndex = pageIndex.toString();  // Add this line
      ...
      for (let i = 0; i < pageCount; i++) {
        if (i === 0) {
          const firstPageClone = firstPage.cloneNode(true) as HTMLElement;
          firstPageClone.dataset.pageIndex = "0";  // Add page index
          fragment.appendChild(firstPageClone);
        } else {
          const pageClone = page.cloneNode(true) as HTMLElement;
          pageClone.dataset.pageIndex = i.toString();  // Add page index
