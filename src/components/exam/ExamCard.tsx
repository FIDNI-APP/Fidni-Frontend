...
export const ExamCard: React.FC<ExamCardProps> = ({
  ...
  useEffect(() => {
    if (exam && exam.user_save !== undefined) {
      setIsSaved(exam.user_save);
    }
  }, [exam]);
  ...
  const handleSaveClick = async (e: React.MouseEvent) => {
    ...
    try {
        ...
        if (isSaved) {
            ...
            await unsaveExercise(exam.id);
            setIsSaved(false);
        } else {
            // If not saved, save it
            try {
                await saveExercise(exam.id);
                setIsSaved(true);
            } catch (error) {
  ...
  // Format a date string for display (national exams)
  ...
  const totalMoreTags = (
    (hasChapters ? Math.max(0, exam.chapters.length - 1) : 0) + 
    (hasSubfields ? Math.max(0, exam.subfields.length - 1) : 0) + 
    (hasTheorems ? Math.max(0, exam.theorems.length - 1) : 0)
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className={`bg-gradient-to-r ${exam.is_national_exam ? 'from-blue-700 to-blue-500' : 'from-indigo-600 to-violet-600'} p-4 relative overflow-hidden`}>
            ...
            <div className="flex justify-between items-start mb-2">
              <div className="flex items-start gap-2">
                {exam.is_national_exam && (
                  <div className="bg-white/20 backdrop-blur-sm px-2 py-1 rounded-md text-white flex items-center flex-shrink-0 mt-1 transition-transform duration-300 transform group-hover:scale-105">
                    <Award className="w-3 h-3 mr-1" />
                    <span className="text-xs font-medium">Examen National 🇫🇷</span>
                  </div>
                )}
                <h2 className="text-xl font-bold text-white leading-tight group-hover:underline decoration-2 decoration-white/60">
                  {truncateTitle(exam.title)}
                </h2>
              </div>
        ...
        <div className="px-5 py-4 flex-grow">
          <div className={`transition-opacity duration-150 ${!contentLoaded ? 'opacity-0 absolute' : 'opacity-100'}`}>
            <div className="prose prose-sm max-w-none text-gray-700 line-clamp-3 overflow-hidden">
              <MemoizedTipTapRenderer content={exam.content} onReady={handleTipTapReady} />
            </div>
          </div>
        ...
        <div className="mt-auto px-5 py-3 border-t border-gray-100 bg-gray-50 flex items-center justify-between transition-colors duration-300 group-hover:bg-white">
          ...
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            <div className="flex items-center flex-wrap gap-1.5 overflow-hidden max-w-xs">
              {/* Chapter Tags */}
              {hasChapters && exam.chapters.slice(0, 1).map(chapter => (
                <span
                  key={chapter.id}
              ...
              {/* Subfield Tags */}
              {hasSubfields && exam.subfields.slice(0, 1).map(subfield => (
                <span
                  key={subfield.id}
              ...
              {/* Theorem Tags */}
              {hasTheorems && exam.theorems.slice(0, 1).map(theorem => (
                <span
                  key={theorem.id}
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={(e) => {
                e.stopPropagation();
                navigate(`/exams/${exam.id}#comments`);
              }}
            ...
            >
              ...
              <span>{(exam.comments || []).length}</span>
            </button>
            ...
            <button
              onClick={(e) => {
                e.stopPropagation();
                navigator.clipboard.writeText(window.location.origin + `/exams/${exam.id}`);
        ...
        >
          ...
          >
            ...
            <p className="font-medium text-lg mb-1 text-white">Voir l'examen</p>
          </button>
        </div>
      </div>
