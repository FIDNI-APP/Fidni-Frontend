...
import { Search, Loader2, BookOpen, TrendingUp, X } from 'lucide-react';
...
export const SearchAutocomplete: React.FC<SearchAutocompleteProps> = ({
  ...
  const getContentIcon = (type: string) => {
    switch (type) {
      case 'lesson':
        return <LessonIcon className="w-4 h-4" />;
      case 'exam':
        return <APlusIcon className="w-4 h-4" />;
      default:
        ...
        <div className="relative group">
          ...
          <button
            ...
            className={`absolute right-2 top-1/2 transform -translate-y-1/2 text-white px-3 py-1.5 rounded-lg transition-all duration-200 hover:scale-105 active:scale-95 
              ${type === 'exercise' ? 'bg-blue-600 hover:bg-blue-700' : 
                type === 'lesson' ? 'bg-emerald-600 hover:bg-emerald-700' : 
              ...
              {results.lessons.length > 0 && (
                <div className="border-b border-gray-100">
                  <div className="px-4 py-2.5 bg-gradient-to-r from-indigo-50 to-blue-50 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <LessonIcon className="w-4 h-4 text-indigo-600" />
                      <span className="text-xs font-semibold text-gray-700 uppercase tracking-wide">
                        Leçons
                      </span>
                    </div>
              ...
              {results.exams.length > 0 && (
                <div className="border-b border-gray-100">
                  <div className="px-4 py-2.5 bg-gradient-to-r from-purple-50 to-pink-50 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <APlusIcon className="w-4 h-4 text-purple-600" />
                      <span className="text-xs font-semibold text-gray-700 uppercase tracking-wide">
                        Examens
                      </span>
                    </div>
